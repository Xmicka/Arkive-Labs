...
export default function Craft() {
  useEffect(() => {
    ...
    // ---- Reveal on scroll ----
    // Curated targets across every section, plus any explicit [data-reveal].
    // We only hide elements that start BELOW the fold, so above-the-fold
    // content never flashes and the hero keeps its own entrance animation.
    const autoSelector = [
      "[data-reveal]",
      ".section-index",
      ".pov-content",
    ...
    const targets = Array.from(
      document.querySelectorAll<HTMLElement>(autoSelector),
    );

    if (targets.length && "IntersectionObserver" in window && !reduce) {
      const vh = window.innerHeight;
      const toReveal = targets.filter((el) => {
        const top = el.getBoundingClientRect().top;
        return top > vh * 0.9; // below the fold on load
      });
